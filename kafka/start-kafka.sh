...
KERBEROS_KAFKA_SERVER_PRIMARY=${KERBEROS_KAFKA_SERVER_PRIMARY:-'kafka'}
KERBEROS_KAFKA_CLIENT_PRIMARY=${KERBEROS_KAFKA_SERVER_PRIMARY:-'kafka'}
KERBEROS_ZOOKEEPER_CLIENT_PRIMARY=${KERBEROS_KAFKA_SERVER_PRIMARY:-'zookeeper'}
...
# Add jaas.conf
cat > $KAFKA_HOME/config/jaas.conf << EOF
KafkaServer {
     com.sun.security.auth.module.Krb5LoginModule required
     useKeyTab=true
     storeKey=true
     keyTab="/kerberos/${KERBEROS_KAFKA_SERVER_PRIMARY}.keytab"
     principal="${KERBEROS_KAFKA_SERVER_PRIMARY}/`hostname -f`@${KERBEROS_REALM}";
};
KafkaClient {
     com.sun.security.auth.module.Krb5LoginModule required
     useKeyTab=true
     storeKey=true
     keyTab="/kerberos/${KERBEROS_KAFKA_CLIENT_PRIMARY}.keytab"
     principal="${KERBEROS_KAFKA_CLIENT_PRIMARY}/`hostname -f`@${KERBEROS_REALM}";
};
Client {
    com.sun.security.auth.module.Krb5LoginModule required
    useKeyTab=true
