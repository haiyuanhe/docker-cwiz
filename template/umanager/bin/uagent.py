...
config.read('<:install_root:>/umanager/config/uagent.conf')
install_root = config.get('envs', 'install_root')
platform = config.get('envs', 'platform')
agent_home = "<:install_root:>/agent/cloudwiz-agent/agent"
...
from collectors.lib.utils import get_hostname
...
class VersionFile:
    ...
    def __str__(self):
        ...
        if hasattr(self, 'version'):
            string += "["
            string += str(self._get_version())
            string += "]"
        if hasattr(self, 'platform'):
            string += "["
            string += str(self._get_platform())
            string += "]"
        if hasattr(self, 'release_date_utc'):
            string += "["
            string += str(self._get_release_date_utc())
            string += "]"
        return string
...
class UAgent:
    ...
    def log_current_version(self):
        """ Log current version into uagent log. """
        try:
            version_filename = os.path.join(install_root, "version.json")
            if os.path.isfile(version_filename):
                version_file = VersionFile(version_filename, self._logger)
                self._logger.log_info("current agent version: %s", str(version_file))
        except:
            pass
    ...
    def run(self):
        ...
        self.log_current_version()

        try:
            org_id = config.get('envs', 'org_id')
            sys_id = config.get('envs', 'sys_id')
            hostname = get_hostname(self._logger)
        except:
            org_id = '0'
            sys_id = '0'
            hostname = 'any'
        base_urls = config.get("urls", "server_base")
        self._logger.log_info("org_id=%s, sys_id=%s, hostname=%s", str(org_id), str(sys_id), str(hostname))

        for base_url in base_urls.split(','):
            # Try host specific URL
            host_specific_url = base_url + "/singles/" + org_id + "/" + sys_id + "/" + hostname
            self._logger.log_info("try host specific url %s", host_specific_url)
            exit_code = self.upgrade_once(host_specific_url)
            self._logger.log_info("finish connect to host specific url %s, return code %d", host_specific_url, exit_code)
            if exit_code == ExitCode.OK:
                return ExitCode.OK

            # Try client specific URL
            client_specific_url = base_url + "/singles/" + org_id + "/" + sys_id + "/" + platform
