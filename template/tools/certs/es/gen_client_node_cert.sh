...
echo "$CLIENT_NAME keystore password :$KS_PASS" >> $CERT_ROOT/Readme.txt
...
cat "$CLIENT_NAME.crt.pem" $CERT_ROOT/ca/chain-ca.pem  > "$CLIENT_NAME.crtfull.pem"

cp -pr $CLIENT_NAME-keystore.jks $CLIENT_NAME.jks
