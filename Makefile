...
build:
	...
	docker build --build-arg PKG_URL=${PKG_URL} --build-arg INSTALL_ROOT=${INSTALL_ROOT} -t cloudwiz/zookeeper:${ZOOKEEPER} ./zookeeper
	docker build -t cloudwiz/kafka:${KAFKA} ./kafka
